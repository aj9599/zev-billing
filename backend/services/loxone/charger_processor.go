...
func (conn *WebSocketConnection) processChargerField(device *Device, response LoxoneResponse, fieldName string, collection *ChargerDataCollection, db *sql.DB) {
	log.Printf("   ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ¢â‚¬Â¹ [%s] Processing field '%s'", device.Name, fieldName)
	log.Printf("   ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã¢â‚¬Â¹ Response Control: %s", response.LL.Control)
	log.Printf("   ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã¢â‚¬Â¹ Response Code: %s", response.LL.Code)
	log.Printf("   ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã¢â‚¬Â¹ Response Value: %s", response.LL.Value)
	log.Printf("   ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã¢â‚¬Â¹ Number of outputs: %d", len(response.LL.Outputs))

	for key := range response.LL.Outputs {
		log.Printf("   ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã¢â‚¬Â¹ Found output key: %s", key)
	}

	if output1, ok := response.LL.Outputs["output1"]; ok {
		log.Printf("   ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã¢â‚¬Â¹ output1 found - Value type: %T, Value: %v", output1.Value, output1.Value)
		switch fieldName {
		case "power":
			var power float64
			...
			case string:
				...
				if f, err := strconv.ParseFloat(cleanValue, 64); err == nil {
					...
					log.Printf("   ÃƒÂ¢Ã…Â¡Ã‚Â ÃƒÂ¯Ã‚Â¸Ã‚Â [%s] Failed to parse power from output1: '%s' (err: %v)", device.Name, v, err)
				}
			}
			collection.Power = &power
			log.Printf("   ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã…Â  [%s] Received power: %.4f kWh", device.Name, power)
		...
		case "state":
			...
			log.Printf("   ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ¢â‚¬â„¢ [%s] Received state: %s", device.Name, state)
		...
		case "user_id":
			...
			log.Printf("   ÃƒÂ°Ã…Â¸Ã¢â‚¬ËœÃ‚Â©ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â§ [%s] Received user_id: %s", device.Name, userID)
		...
		case "mode":
			...
			log.Printf("   ÃƒÂ¢Ã…Â¡Ã¢â€žÂ¢ÃƒÂ¯Ã‚Â¸Ã‚Â [%s] Received mode: %s", device.Name, mode)
		}

		hasAll := collection.Power != nil && collection.State != nil &&
			collection.UserID != nil && collection.Mode != nil

		log.Printf("   ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã‚Â¦ [%s] Collection status: Power=%v State=%v UserID=%v Mode=%v (Complete=%v)",
			device.Name,
		...
		if hasAll {
			log.Printf("   ÃƒÂ¢Ã…â€œÃ¢â‚¬ÂÃƒÂ¯Ã‚Â¸Ã‚Â [%s] All fields collected, saving to database", device.Name)
		...
		log.Printf("   ÃƒÂ¢Ã…Â¡Ã‚Â ÃƒÂ¯Ã‚Â¸Ã‚Â [%s] output1 not found in response for field '%s'", device.Name, fieldName)

		if response.LL.Value != "" {
			log.Printf("   ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã¢â‚¬Â¹ Trying to use response.LL.Value: %s", response.LL.Value)
			...
			case "power":
				...
				if f, err := strconv.ParseFloat(cleanValue, 64); err == nil {
					collection.Power = &f
					log.Printf("   ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã…Â  [%s] Received power from Value: %.4f kWh (from '%s')", device.Name, f, response.LL.Value)
				} else {
					log.Printf("   ÃƒÂ¢Ã‚ÂÃ…â€™ [%s] Failed to parse power from Value: '%s' (err: %v)", device.Name, response.LL.Value, err)
				}
			case "state":
				state := response.LL.Value
				collection.State = &state
				log.Printf("   ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ¢â‚¬â„¢ [%s] Received state from Value: %s", device.Name, state)
			case "user_id":
				userID := response.LL.Value
				collection.UserID = &userID
				log.Printf("   ÃƒÂ°Ã…Â¸Ã¢â‚¬ËœÃ‚Â©ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â§ [%s] Received user_id from Value: %s", device.Name, userID)
			case "mode":
				mode := response.LL.Value
				collection.Mode = &mode
				log.Printf("   ÃƒÂ¢Ã…Â¡Ã¢â€žÂ¢ÃƒÂ¯Ã‚Â¸Ã‚Â [%s] Received mode from Value: %s", device.Name, mode)
			}
			...
			log.Printf("   ÃƒÂ¢Ã‚ÂÃ…â€™ [%s] No data found for field '%s' in response", device.Name, fieldName)
		}
	}
}
...
func (conn *WebSocketConnection) processChargerSingleBlock(device *Device, response LoxoneResponse, db *sql.DB, collector LoxoneCollectorInterface) {
	log.Printf("   ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ¢â‚¬Â¹ [%s] Processing single-block response (enhanced session tracking)", device.Name)
	log.Printf("   ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã‚Â¦ Number of outputs: %d", len(response.LL.Outputs))
	...
	if output1, ok := response.LL.Outputs["output1"]; ok {
		...
		log.Printf("      ÃƒÂ¢Ã¢â‚¬ÂÃ…â€œÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ output1 (Vc - Vehicle Connected): %d", vehicleConnected)
	}
	...
	if output2, ok := response.LL.Outputs["output2"]; ok {
		...
		log.Printf("      ÃƒÂ¢Ã¢â‚¬ÂÃ…â€œÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ output2 (Cac - Charging Active): %d", chargingActive)
	}
	...
	if output3, ok := response.LL.Outputs["output3"]; ok {
		...
		log.Printf("      ÃƒÂ¢Ã¢â‚¬ÂÃ…â€œÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ output3 (Cp - Charging Power): %.3f kW", chargingPowerKW)
	}
	...
	if output4, ok := response.LL.Outputs["output4"]; ok {
		...
		log.Printf("      ÃƒÂ¢Ã¢â‚¬ÂÃ…â€œÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ output4 (M - Mode): %s", modeValue)
	}
	...
	if output7, ok := response.LL.Outputs["output7"]; ok {
		...
		log.Printf("      ÃƒÂ¢Ã¢â‚¬ÂÃ…â€œÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ output7 (Mr - Total Energy): %.3f kWh", totalEnergyKWh)
	}
	...
	if output8, ok := response.LL.Outputs["output8"]; ok {
		...
		log.Printf("      ÃƒÂ¢Ã¢â‚¬ÂÃ…â€œÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ output8 (Ccc - Current Session Energy): %.3f kWh", currentSessionEnergyKWh)
	}
	...
	if output9, ok := response.LL.Outputs["output9"]; ok {
		...
		log.Printf("      ÃƒÂ¢Ã¢â‚¬ÂÃ…â€œÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ output9 (Clc - Last Session Energy): %.3f kWh", lastSessionEnergyKWh)
	}
	...
	if output11, ok := response.LL.Outputs["output11"]; ok {
		...
		log.Printf("      ÃƒÂ¢Ã¢â‚¬ÂÃ…â€œÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ output11 (Cld - Last Session Duration): %.3f hours (%.0f seconds)", lastSessionDurationSec/3600, lastSessionDurationSec)
	}
	...
	if output12, ok := response.LL.Outputs["output12"]; ok {
		...
		log.Printf("      ÃƒÂ¢Ã¢â‚¬ÂÃ…â€œÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ output12 (Cw - Weekly Energy): %.3f kWh", weeklyEnergyKWh)
	}
	...
	if output13, ok := response.LL.Outputs["output13"]; ok {
		...
		log.Printf("      ÃƒÂ¢Ã¢â‚¬ÂÃ…â€œÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ output13 (Cm - Monthly Energy): %.3f kWh", monthlyEnergyKWh)
	}
	...
	if output14, ok := response.LL.Outputs["output14"]; ok {
		...
		log.Printf("      ÃƒÂ¢Ã¢â‚¬ÂÃ…â€œÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ output14 (Clm - Last Month Energy): %.3f kWh", lastMonthEnergyKWh)
	}
	...
	if output15, ok := response.LL.Outputs["output15"]; ok {
		...
		log.Printf("      ÃƒÂ¢Ã¢â‚¬ÂÃ…â€œÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ output15 (Cy - Yearly Energy): %.3f kWh", yearlyEnergyKWh)
	}
	...
	if output16, ok := response.LL.Outputs["output16"]; ok {
		...
		log.Printf("      ÃƒÂ¢Ã¢â‚¬ÂÃ…â€œÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ output16 (Cly - Last Year Energy): %.3f kWh", lastYearEnergyKWh)
	}
	...
	if output17, ok := response.LL.Outputs["output17"]; ok {
		...
		log.Printf("      ÃƒÂ¢Ã¢â‚¬ÂÃ…â€œÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ output17 (Lcl - Last Session Log): %s", lastSessionLog)
	}
	...
	if output21, ok := response.LL.Outputs["output21"]; ok {
		...
		log.Printf("      ÃƒÂ¢Ã¢â‚¬ÂÃ¢â‚¬ÂÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ output21 (Uid - User ID): '%s'", userID)
	}
	...
	if collector != nil {
		...
		if sessionActive {
			if activeSession == nil {
				...
				log.Printf("   ÃƒÂ¢Ã…Â¡Ã‚Â¡ [%s] SESSION STARTED - Creating new session", device.Name)
				...
				if err := collector.SaveActiveSessionToDatabase(newSession); err != nil {
					log.Printf("   Ã¢Å¡Â Ã¯Â¸Â  [%s] Failed to persist session: %v", device.Name, err)
				}
				...
				if err := collector.SaveActiveSessionToDatabase(activeSession); err != nil {
					log.Printf("   Ã¢Å¡Â Ã¯Â¸Â  [%s] Failed to update persisted session: %v", device.Name, err)
				}

				log.Printf("   ÃƒÂ¢Ã…Â¡Ã‚Â¡ [%s] SESSION ONGOING: Reading added - Energy: %.3f kWh, Power: %.2f kW, Readings: %d",
			...
			if activeSession != nil {
				...
				if lclChanged {
					log.Printf("   ÃƒÂ°Ã…Â¸Ã‚ÂÃ‚Â [%s] SESSION ENDED - Lcl changed, processing completed session", device.Name)
					...
					if energyDelta > 0.1 {
						log.Printf("   ÃƒÂ¢Ã‚ÂÃ‚Â³ [%s] Session paused (ÃƒÅ½Ã¢â‚¬Â%.3f kWh) - waiting for Lcl update", device.Name, energyDelta)
					} else {
						// Very small energy, discard session
						log.Printf("   ÃƒÂ°Ã…Â¸Ã¢â‚¬â€Ã¢â‚¬ËœÃƒÂ¯Ã‚Â¸Ã‚Â [%s] Session discarded (ÃƒÅ½Ã¢â‚¬Â%.3f kWh too small)", device.Name, energyDelta)
						collector.DeleteActiveSession(device.ID)
					}
				}
			} else {
	...
	log.Printf("   ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ [%s] Live data updated: Energy=%.3f kWh, Power=%.2f kW, State=%s (%s), Mode=%s",
...
func UpdateChargerStatsInDatabase(db *sql.DB, chargerID int, lastSessionEnergy, lastSessionDuration,
	...
	if err != nil {
		...
		log.Printf("   ÃƒÂ°Ã…Â¸Ã¢â‚¬â„¢Ã‚Â¾ [Charger %d] Stats saved to charger_stats table", chargerID)
	}
}
...
func WriteMaintenanceReading(chargerID int, chargerName string, totalEnergy float64, mode string, db *sql.DB, collector LoxoneCollectorInterface) {
	...
	`, chargerID, currentTime.Format("2006-01-02 15:04:05-07:00")).Scan(&exists)
	...
	_, err = db.Exec(`
		...
		VALUES (?, ?, ?, ?, ?, ?)
	`, chargerID, "", currentTime.Format("2006-01-02 15:04:05-07:00"),
		totalEnergy, mode, "1") // state = 1 (disconnected, like Zaptec)

	if err != nil {
		log.Printf("   ÃƒÂ¢Ã…Â¡Ã‚Â ÃƒÂ¯Ã‚Â¸Ã‚Â [%s] Failed to write maintenance reading: %v", chargerName, err)
	} else {
		log.Printf("   ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã‚Â [%s] Maintenance reading written: %.3f kWh at %s (disconnected, no user)",
...
func (conn *WebSocketConnection) saveChargerDataLegacy(device *Device, collection *ChargerDataCollection, db *sql.DB) {
	...
	if err == nil && !lastTime.IsZero() {
		...
		if len(interpolated) > 0 {
			...
			log.Printf("   ÃƒÂ¢Ã…Â¡Ã‚Â ÃƒÂ¯Ã‚Â¸Ã‚Â Filled %d reading gaps for charger %s", len(interpolated), device.Name)
		}
	}
	...
	if err != nil {
		log.Printf("ÃƒÂ¢Ã‚ÂÃ…â€™ Failed to save charger session to database: %v", err)
		conn.Mu.Lock()
		conn.LastError = fmt.Sprintf("DB save failed: %v", err)
		conn.Mu.Unlock()
	} else {
		log.Printf("ÃƒÂ¢Ã…â€œÃ¢â‚¬ÂÃƒÂ¯Ã‚Â¸Ã‚Â CHARGER [%s]: %.4f kWh (user: %s, mode: %s, state: %s)",
		...
		db.Exec(`
			...
			SET notes = ?
			WHERE id = ?
		`, fmt.Sprintf("ÃƒÂ°Ã…Â¸Ã…Â¸Ã‚Â¢ Last update: %s", time.Now().Format("2006-01-02 15:04:05")),
			device.ID)
	}
}
