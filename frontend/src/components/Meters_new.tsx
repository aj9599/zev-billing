...
import { Plus, Edit2, Trash2, X, Info, HelpCircle, Zap, Download, Search, Building, Radio, Settings, AlertCircle, Star, Wifi, WifiOff, AlertTriangle, RefreshCw, Archive, Rss } from 'lucide-react';
...
// Extract the modal as a separate memoized component to prevent re-renders
...
export default function Meters() {
    ...
    const isMqttTopicUsed = (topic: string): boolean => {
        return meters.some(meter => {
    ...
    const generateUniqueMqttTopic = (meterName: string): string => {
        const safeName = meterName.toLowerCase().replace(/[^a-z0-9]/g, '_');
        let topic = `meters/${safeName}`;
        let counter = 1;

        while (isMqttTopicUsed(topic)) {
    ...
    const getMqttConnectionStatus = (meterId: number) => {
        return mqttStatus[meterId];
    };
    ...
    const renderConnectionStatus = (meter: Meter) => {
        ...
        } else if (meter.connection_type === 'mqtt') {
            // ADD THIS:
            const status = getMqttConnectionStatus(meter.id);
            if (status) {
                return (
                    <div style={{
                        display: 'flex',
                            ...
                            <>
                                ...
                                <div style={{ flex: 1 }}>
                                    ...
                                    <div style={{ fontSize: '11px', color: '#6b7280' }}>
                                        Last update: {new Date(status.last_update).toLocaleTimeString('de-CH', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false })}
                                    </div>
                                </div>
                            </>
                        ) : (
        ...
        <div style={{
            ...
            <div className="modal-content" style={{
                ...
                <div style={{ lineHeight: '1.8', color: '#374151' }}>
                    ...
                    <div style={{ backgroundColor: '#f3e8ff', padding: '16px', borderRadius: '8px', marginBottom: '16px', border: '2px solid #8b5cf6' }}>
                        <p style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
                    ...
                    <div style={{ backgroundColor: '#fef3c7', padding: '16px', borderRadius: '8px', border: '1px solid #f59e0b' }}>
                        <ul style={{ marginLeft: '20px' }}>
                            ...
                            <li><strong>MQTT:</strong> Check if MQTT broker is running: <code style={{ backgroundColor: '#fff', padding: '2px 6px', borderRadius: '4px' }}>sudo systemctl status mosquitto</code></li>
                            <li><strong>MQTT:</strong> Test topic subscription: <code style={{ backgroundColor: '#fff', padding: '2px 6px', borderRadius: '4px' }}>mosquitto_sub -h localhost -t "meters/#"</code></li>
                            <li><strong>MQTT:</strong> Check device is publishing: <code style={{ backgroundColor: '#fff', padding: '2px 6px', borderRadius: '4px' }}>mosquitto_pub -h localhost -t "test" -m "hello"</code></li>
                        ...
                        <form onSubmit={handleSubmit}>
                            <div>
                                ...
                                <input type="text" required value={formData.name} onChange={(e) => {
                                    ...
                                    // Auto-generate MQTT topic when name changes (only for new meters)
                                    if (!editingMeter && formData.connection_type === 'mqtt') {
                                        const topic = generateUniqueMqttTopic(e.target.value);
                                        setConnectionConfig({ ...connectionConfig, mqtt_topic: topic });
                                    }
                                }}
                            ...
                            <div style={{ marginTop: '16px' }}>
                                ...
                                <select required value={formData.connection_type} onChange={(e) => {
                                    setFormData({ ...formData, connection_type: e.target.value });
                                    // Auto-generate MQTT topic when switching to MQTT
                                    if (e.target.value === 'mqtt' && formData.name) {
                                        const topic = generateUniqueMqttTopic(formData.name);
                                        setConnectionConfig({ ...connectionConfig, mqtt_topic: topic });
                                    }
                                }}
                                    style={{ width: '100%', padding: '10px', border: '1px solid #ddd', borderRadius: '6px' }}>
                            ...
                            <div style={{ marginTop: '20px', padding: '20px', backgroundColor: '#f9f9f9', borderRadius: '8px' }}>
                                ...
                                {formData.connection_type === 'mqtt' && (
                                    <>
                                        <div style={{ backgroundColor: '#f3e8ff', padding: '12px', borderRadius: '6px', marginBottom: '12px', border: '1px solid #8b5cf6', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                        ...
                                        <div style={{ marginBottom: '12px' }}>
                                            ...
                                            <input type="text" required value={connectionConfig.mqtt_topic || ''}
                                                onChange={(e) => setConnectionConfig({ ...connectionConfig, mqtt_topic: e.target.value })}
                                        ...
                                        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '12px', marginBottom: '12px' }}>
                                            <div>
                                                ...
                                                <input type="text" required value={connectionConfig.mqtt_broker || ''}
                                                    onChange={(e) => setConnectionConfig({ ...connectionConfig, mqtt_broker: e.target.value })}
                                            ...
                                            <div>
                                                ...
                                                <input type="number" required value={connectionConfig.mqtt_port}
                                                    onChange={(e) => setConnectionConfig({ ...connectionConfig, mqtt_port: parseInt(e.target.value) })}
                                                    placeholder="1883"
                                                    style={{ width: '100%', padding: '10px', border: '1px solid #ddd', borderRadius: '6px' }} />
                                            </div>
                                        </div>
                                        ...
                                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
                                            <div>
                                                ...
                                                <input type="text" value={connectionConfig.mqtt_username || ''}
                                                    onChange={(e) => setConnectionConfig({ ...connectionConfig, mqtt_username: e.target.value })}
                                            ...
                                            <div>
                                                ...
                                                <input type="password" value={connectionConfig.mqtt_password || ''}
                                                    onChange={(e) => setConnectionConfig({ ...connectionConfig, mqtt_password: e.target.value })}
                                                    placeholder="••••••••"
                                                    style={{ width: '100%', padding: '10px', border: '1px solid #ddd', borderRadius: '6px' }} />
                                            </div>
                                        </div>
                                        ...
                                        <div style={{ marginBottom: '12px' }}>
                                            ...
                                            <select value={connectionConfig.mqtt_qos}
                                                onChange={(e) => setConnectionConfig({ ...connectionConfig, mqtt_qos: parseInt(e.target.value) })}
                                                style={{ width: '100%', padding: '10px', border: '1px solid #ddd', borderRadius: '6px' }}>
